...
namespace NHibernate.SolrNet.NHSearchMapping.AttributeBased
{
    ...
    public class AttributeSearchMappingBuilder
    {
        ...
        public class BuildContext
        {
            ...
            public ISet<Type> Processed { get; private set; }
        }
        ...
        //private FilterKey BuildFilterDef(FullTextFilterDefAttribute attribute)
        //{
        ...
        private void BuildProperty(DocumentMapping documentMapping, MemberInfo member, /*Analyzer parentAnalyzer,*/ bool isRoot, string path, BuildContext context)
        {
            ...
            if (documentIdAttribute != null)
            {
                ...
                if (isRoot)
                {
                    ...
                    {
                        throw new NHMappingException("Bridge for document id does not implement TwoWayFieldBridge: " + member.Name);
                    }

                    documentMapping.DocumentId = new DocumentIdMapping(documentIdName, member.Name, (ITwoWayFieldBridge)bridge, getter)
                    {
                        Boost = boost
                    };
                }
            ...
            if (fieldAttributes.Length > 0)
            {
                ...
                foreach (var fieldAttribute in fieldAttributes)
                {
                    ...
                    var field = new FieldMapping(GetAttributeName(member, fieldAttribute.Name), bridge, getter)
                    {
